            ...
            <table class="table table-sm table-hover align-middle">
              ...
              <tbody>
                ...
                <tr v-for="(entry, index) in riwayatKronologi" :key="index" :class="{ 'rowActive': selectedRow === entry, 'rowActive': selectedRow && selectedRow.no_transaksi === entry.no_transaksi }" @click="getDetailPasien(entry.no_transaksi); selectRow(entry)">
                  ...
                  <td><span class="badge text-secondary ms-1 border">{{ JSON.parse(entry.dibuat_oleh).username }}</span></td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      <div v-if="selectedRow" class="row">
        <div class="col-12 no-print">
          <div class="d-flex align-items-center mb-3">
            <button type="button" class="btn btn-sm btn-dark me-2" @click="selectedRow = null"><i class="fas fa-chevron-left"></i> <span class="">Kembali</span> </button> 
            <span>
              <b>{{ detailPasien ? detailPasien.KD_PASIEN : '' }} </b> - <b class="text-primary text-decoration-underscore"> {{ detailPasien ? detailPasien.NAMAPASIEN : '' }} </b>
            </span>
          </div>
        </div>
        <div v-if="selectedRow" class="col-md-4 no-print">
          ...
          <small>dibuat oleh: {{ JSON.parse(selectedRow.dibuat_oleh).role }} - {{ JSON.parse(selectedRow.dibuat_oleh).username }} </small>
          <div class="mb-5">
            <div
              v-if="loading"
          ...
          <div v-if="detailPasien.KPNO_TRANSAKSI" class="">
            ...
            <form ref="form" id="formInsiden" @submit.prevent="submitForm">
              ...
              <div class="tab-content mb-5">
                <div
                  class="tab-pane p-3 bg-white active shadow mb-4"
                  id="data-pasien"
                  role="tabpanel"
              ...
              <div class="text-center mb-4 p-3 no-print">
                <button type="button" class="btn btn-dark me-2" @click="selectedRow = null"><i class="fas fa-chevron-left"></i> <span class="">Kembali</span> </button> 
                <div class="btn-group">
