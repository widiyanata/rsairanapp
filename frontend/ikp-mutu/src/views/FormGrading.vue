...
const getKronologi = async () => {
  loading.value = true
  try {
    const res = await fetch('http://localhost:8009/kronologi')
...
const getDetailPasien = async (no_transaksi) => {
  loading.value = true
  try {
    const res = await fetch(`http://localhost:8009/kunjunganPasien?cari=${no_transaksi}`)
...
const submitForm = async (e) => {
  ...
  const res = await fetch('http://localhost:8009/grading', {
    method: 'POST',
    headers: {
...
const getRiwayatGrading = async (no_transaksi = '') => {
  ...
  const res = await fetch('http://localhost:8009/grading?no_transaksi='+ no_transaksi)
  const data = await res.json()
